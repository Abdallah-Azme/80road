...
import { Category, CategoryValue } from '@/features/post-ad/services/post-ad.service';
...
// ── Shared helpers ──────────────────────────────────────────────────────────
function ProgressTop({ step, totalSteps, setStep }: { step: number; totalSteps: number; setStep: (s: number) => void }) {
  return (
      ...
      <div className="flex flex-col gap-6 py-6 md:py-10" dir="rtl">
        <div className="flex items-center justify-between gap-6">
            <div className="flex flex-col gap-1.5 min-w-fit">
              <span className="text-[10px] md:text-xs font-black text-muted-foreground uppercase tracking-[0.2em]">الخطوة {step} من {totalSteps}</span>
...
function Opt({ label, selected, onClick, sub }: { label: React.ReactNode; selected: boolean; onClick: () => void; sub?: string }) {
  return (
    <button type="button" onClick={onClick}
      ...
      <div className="flex flex-col gap-1">
        ...
        {sub && <div className="text-sm text-muted-foreground font-bold">{sub}</div>}
      </div>
      <div className={cn(
...
function PostAdWizard() {
  ...
  const sel = (key: string, value: unknown) => { handleUpdate(key, value); setTimeout(next, 150); };
        ...
        >
          <div className="max-w-4xl mx-auto px-5 w-full">
            ...
            {currentStepInfo.type === 'category' && (
              <FormField control={form.control} name={`category_values_ids.${currentStepInfo.data.id}`} render={({ field }) => (
              ...
              <FormField control={form.control} name="country" render={({ field }) => (
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {countries?.map(c =>
                      <Opt key={c.id} label={<span className="flex items-center gap-3"><Image src={c.image} width={32} height={20} alt={c.name} className="rounded-sm object-cover" />{c.name}</span>}
                        selected={field.value === c.id} onClick={() => {
                ...
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                    {states?.map((o: { id: number; name: string }) =>
                      <Opt key={o.id} label={o.name} selected={field.value === o.id} onClick={() => {
                        handleUpdate('area', '');
                        sel('governorate', o.id);
                      }} />)}
                    {(!states || states.length === 0) && (
                       <div className="col-span-full py-20 text-center text-muted-foreground font-bold">لا يوجد محافظات متوفرة لهذه الدولة</div>
                    )}
                  </div>
                  <FormMessage className="text-center font-black mt-4" />
                </div>
                ...
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 h-[400px] md:h-auto overflow-y-auto no-scrollbar pb-10 text-right" dir="rtl">
                    ...
                    {(!cities || cities.length === 0) && (
                       <div className="col-span-full py-20 text-center text-muted-foreground font-bold">لا يوجد مدن متوفرة لهذه المحافظة</div>
                    )}
                  </div>
                  <FormMessage className="text-center font-black mt-4" />
                </div>
                ...
                <div className="bg-card rounded-3xl p-6 md:p-8 shadow-xl shadow-primary/5 border border-border mb-8 text-base flex flex-col gap-4">
                  ...
                  {categories.map(cat => {
                    ...
                    if (cat.type === 'select' || cat.type === 'boolean') {
                       displayVal = cat.values.find(v => v.id === val)?.value || '—';
            ...
            {/* Location Summary */}
                  <div className="flex justify-between items-center border-b border-border/50 pb-3">
                    ...
                    <span className="font-black text-foreground text-lg">{countries?.find(c => c.id === form.watch('country'))?.name || '—'}</span>
                  </div>
                  <div className="flex justify-between items-center border-b border-border/50 pb-3">
                    <span className="text-muted-foreground font-bold">المحافظة</span>
                    <span className="font-black text-foreground text-lg">{states?.find(s => s.id === form.watch('governorate'))?.name || '—'}</span>
                  </div>
                  <div className="flex justify-between items-center border-b border-border/50 pb-3">
                    <span className="text-muted-foreground font-bold">المنطقة</span>
                    <span className="font-black text-foreground text-lg">{cities?.find(c => c.id === form.watch('area'))?.name || '—'}</span>
                  </div>
        ...
        <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-4xl p-5 md:px-8 bg-linear-to-t from-background via-background to-transparent z-30">
          <div className="bg-card/80 backdrop-blur-xl border border-border p-4 md:p-6 rounded-[2.5rem] shadow-2xl flex flex-col md:flex-row-reverse gap-3">
            ...
            {currentStepInfo.type === 'summary' ? (
              ...
              const type = currentStepInfo.type === 'category' ? currentStepInfo.data.type : currentStepInfo.type;
              if (['range', 'video', 'images'].includes(type)) {
                return (
              ...
              return <div className="hidden md:block flex-2" />;
            })()}
